...
class FactorModel(nn.Module):
    ...
    def forward(self, previous_covariates, previous_treatments, current_covariates):
            ...
            # 调试：输出 hidden_confounders 和 covariates 的维度
            #print(f"hidden_confounders shape: {hidden_confounders.shape}")
            ...
            # 调试：填充后的维度
            #print(f"hidden_confounders shape after padding: {p_hidden_confounders.shape}")
            #print(f"covariates shape after padding: {covariates.shape}")
...
'''
    def forward(self, previous_covariates, previous_treatments, current_covariates):
        ...
        if hidden_confounders.size(0) > covariates.size(0):
            ...
            padding_size = covariates.size(0) - hidden_confounders.size(0)
            hidden_confounders = F.pad(hidden_confounders, (0, 0, 0, padding_size))

        
        # 调试：填充后的维度
        print(f"hidden_confounders shape after padding: {hidden_confounders.shape}")
        print(f"covariates shape after padding: {covariates.shape}")
