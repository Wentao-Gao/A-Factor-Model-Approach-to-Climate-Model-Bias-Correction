...
def time_series_deconfounder(dataset, num_substitute_confounders, exp_name, dataset_with_confounders_filename,
    ...
    num_time_steps = dataset['covariates'].shape[1]

    # Define the split indices
    train_end = int(num_time_steps * 0.7)  # 70% for training
    val_end = int(num_time_steps * 0.85)  # 15% for validation, 15% for testing

    # Split dataset based on time indices
    dataset_map = get_dataset_splits(dataset, train_end, val_end, use_predicted_confounders=False)
